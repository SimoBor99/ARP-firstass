...
void reset_handler(int signum) {
	if (resetting == 1) {
		write_log("RESET IN CORSO, NOT STARTED!","degubx.txt");
		sleep(2);
		if (stop_reset == 1) {
			write_log("STOPPASTI!","degubx.txt");
			debugger = 1;
		}
	}
	else {
		sleep(2);
		resetting = 1;
		write_log("RESET STARTED","debugx.txt");
		velx=1.0;
